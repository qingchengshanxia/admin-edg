 ...
 * @LastEditors: lhl
 * @LastEditTime: 2019-10-24 10:27:28
 -->
<template>
  <el-container :class="{ 'change-big': boxchange, 'hide-aside': hideAside,'body-iscollips':iscollips }" :style="{ height: pageHeight }">
    <el-header>
    ...
    <el-container class="home-body" :style="{ height: containerHeight }">
      ...
      <el-main :style="{ height: containerHeight }">
        <div class="main-head-container clearfix">
          <div class="tag-div" v-show="isShowTagdiv">
            <span class="expand" @click="switchExpand"><i :class="{'iconfont':true,'icon-outdent':!iscollips,'icon-indent':iscollips}"></i></span>
            <div>
              ...
              <div class="main-head" :style="{
                ...
                <div>
                  <el-tag v-for="(tag, index) in tags" :key="tag.id" :class="{ active: curMenuId == tag.id }" @click.native="goThisPage(tag)">
                    <span>{{ tag.title }}</span>
...
export default {
  ...
  methods: {
    switchExpand() {
      //切换菜单栏是展开还是搜索；
      this.iscollips = !this.iscollips;
      this.$refs.aside.switchExpand();
    },
    setCurtagPosition(tag) {
      ...
      if (document.querySelectorAll(".el-tag").length > 0) {
        ...
        if (!isHas) {
          leftW = document.querySelectorAll(".el-tag")[this.tags.length - 1]
            .offsetLeft;
          curTagW = document.querySelectorAll(".el-tag")[this.tags.length - 1]
            .offsetWidth;
          if (leftW + curTagW > this.divW - 30) {
    ...
    isShowSlide(index) {
      ...
      let w = 0;
      for (
        let i = document.querySelectorAll(".el-tag").length - 1;
        i >= 0;
        i--
      ) {
        if (index != i) {
    ...
    goLeft() {
      ...
      if (w - this.divW < Math.abs(parseInt(this.goleft)) - 40) {
    ...
    hiddenAsideDiv() {
      ...
      this.iscollips = false;
      this.$refs.aside.isCollapse = false;
      if (this.hideAside) {
    ...
    setTableHeight() {
      ...
      if (this.isShowTagdiv) {
        ...
        if (this.boxchange) {
          ...
          this.$store.commit(
            "setTableHeightNofullNotag",
            tableHeightNofullNotag
          );
        }
      }
    },
    switchWidthChange() {
      //浏览器窗口的宽度变化时 菜单栏产生相应变化
      if (this.clientWidth < 1380 && !this.boxchange) {
        if (!this.iscollips && !this.$refs.aside.isCollapse) {
          this.iscollips = true;
          this.$refs.aside.isCollapse = true;
        }
      } else if (!this.boxchange) {
        if (this.iscollips && this.$refs.aside.isCollapse) {
          this.iscollips = false;
          this.$refs.aside.isCollapse = false;
        }
    ...
    switchHeightChange() {
      //浏览器窗口的高度变化时 内容的高度产生相应变化
      let clientH = this.clientHeight;
      ...
      if (localStorage.getItem("isFullscreen")) {
        this.boxchange =
          localStorage.getItem("isFullscreen") == "T" ? true : false;
        if (this.boxchange) {
          this.containerHeight = this.clientH + "px";
          this.mainHeight = this.clientH - 52 + "px";
        } else {
      ...
      if (
        sessionStorage.getItem("hideaside") &&
        sessionStorage.getItem("hideaside") == "T"
      ) {
        this.hideAside = true;
      } else {
  ...
  created() {
    ...
    this.switchHeightChange();
  },
  beforeMount() {},
  mounted() {
    //如果菜单目录的标签历史缓存存在，则显示，并切换到标签最后一个的对应页面
    if (
      localStorage.getItem("tagmenu") &&
      localStorage.getItem("activetagmenu")
    ) {
  ...
  watch: {
    ...
    clientWidth: function(val, oldval) {
      this.switchWidthChange();
    },
    clientHeight: function(val, oldval) {
      this.switchHeightChange();
    }
  }
};
