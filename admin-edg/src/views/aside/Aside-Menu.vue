 ...
 * @LastEditors: lhl
 * @LastEditTime: 2019-10-24 10:04:07
 -->
...
<template>
  ...
  <div class="aside-box">
    ...
    <div class="aside-menu" :class="{'unexpand':isCollapse}">
      <el-menu :default-active="defaultAactiveIndex" :default-openeds="openeds" class="zl-aside-menu" :collapse="isCollapse" :unique-opened="true">
        <!-- 一级 -->
        <el-submenu v-if="!!nav1.children.length" v-for="(nav1, index1) in menus" :index="switchString(nav1.meta.id)" :key="nav1.meta.id" :class="{ 'one-menu': true }">
          <!-- 一级目录 -->
          <template slot="title">
          ...
          <div v-for="(nav2, index2) in nav1.children" :key="nav2.meta.id">
            <!--含有三级的二级 -->
            <el-submenu v-if="!!nav2.children.length" :index="switchString(nav2.meta.id)" :key="nav2.meta.id" :class="{ 'two-menu-group': true }">
              <template slot="title">
                <span slot="title">
              ...
              <!-- 三级 -->
              <el-menu-item-group v-for="(nav3, index3) in nav2.children" :key="nav3.meta.id" class="three-menu">
                <el-menu-item :class="setonlyClass(nav3.meta.id)" :index="switchString(nav3.meta.id)" @click="changeTitle(nav3.meta)">
            ...
            <el-menu-item-group v-else class="two-menu">
              <el-menu-item :class="setonlyClass(nav2.meta.id)" :index="switchString(nav2.meta.id)" @click="changeTitle(nav2.meta)">
        ...
        <!-- 只有一级菜单的 -->
        <el-menu-item v-for="(nav1, index1) in menus" v-if="!!!nav1.children.length" :index="switchString(nav1.meta.id)" :key="nav1.meta.id" :class="{ 'one-menu': true }" @click="changeTitle(nav1.meta, 1)">
          <i :class="nav1.meta.icon"></i>
          <span slot="title">{{ nav1.meta.title }}</span>
        </el-menu-item>
      </el-menu>
...
export default {
  ...
  methods: {
    ...
    initCurmenu() {
      //页面首次渲染，初始化显示被选中的菜单
      this.defaultAactiveIndex =
        this.menus[0].children.length > 0
          ? this.menus[0].children[0].id + "-"
          : this.menus[0].meta.id; //页面的id+'-'
      this.changeTitle(
        this.menus[0].children.length > 0
          ? this.menus[0].children[0].meta
          : this.menus[0].meta
      );
    },
