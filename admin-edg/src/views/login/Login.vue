 ...
 * @LastEditors: lhl
 * @LastEditTime: 2019-10-24 09:38:52
 -->
<template>
  <div class="login-container fullscreen">
    <!-- 背景特性粒子 -->
    <vue-particles color="#fefefe" :particleOpacity="0.3" :particlesNumber="80" shapeType="circle" :particleSize="4" linesColor="#eee" :linesWidth="1" :lineLinked="true" :lineOpacity="0.1" :linesDistance="100" :moveSpeed="2" :hoverEffect="false" hoverMode="grab" :clickEffect="true" clickMode="push" class="particle fullscreen"></vue-particles>
      ...
      <div class="login-contain">
        ...
        <div class="login-username" :class="{ active: showitem == 0 }">
          <p @keyup.enter="goLogin">
            <el-input placeholder="请输入用户名" prefix-icon="icon-yonghu iconfont" @keyup.native="trimUserName" v-model="username"></el-input>
          ...
          <p @keyup.enter="goLogin">
            <el-input placeholder="请输入登录密码" prefix-icon="icon-suo iconfont" maxlength="18" type="password" @keyup.native="trimPwd" v-model="password"></el-input>
            <el-tooltip class="item" effect="dark" content="忘记密码了？" placement="right">
              <span @click="forgetPwd" class="active">
...
export default {
  ...
  methods: {
    ...
    goLogin() {
      ...
      //无登录接口
      // if (this.username == "admin" && this.password == 88888888) {
      //   let data = {};
      //   data.token = "";
      //   data.role = 1;
      ...
      //   this.$router.replace({ path: "/shuju/shujuguanli" });
      ...
      this.$http({
        ...
        data: {
          ...
          password: this.$md5(this.username + this.password) //
        },
        success: function(res) {
          let data = res.data;
          delete data.password;
          data.role = 1;
          data.token = res.data.OfferToken;
          localStorage.setItem("hasLogin", true);
          localStorage.setItem("userName", this.username);
          sessionStorage.setItem("userInfo", JSON.stringify(data));
          // this.$router.replace({ path: "/home" });
