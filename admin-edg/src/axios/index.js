...
// let reaUrl = "http://127.0.0.1:8001";//线上请求地址
let reaUrl = "http://58.215.179.98:8086";//线上请求地址
...
Vue.prototype.$http = function (option) {
  ...
  if (option.url.indexOf('/login') == -1) {
    ...
    this.$axios.defaults.headers.common['OfferToken'] = JSON.parse(sessionStorage.getItem('userInfo')).token;
  } else {
  ...
  this.$axios({
    ...
    url: option.noservices ? url : (isProduct ? reaUrl+url : '/api' + url),
    data: data,
